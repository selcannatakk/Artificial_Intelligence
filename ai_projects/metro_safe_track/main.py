...
def main():
    ...
    writer_video = cv2.VideoWriter(os.path.join("results/realtime_od", "MetroSafeTrack.mp4"), fourcc, 20.0,
    ...
    while ret:
        ...
        # x_factor = 640 / image_height
        # y_factor = 480 / image_width
        ...
        if os.path.exists(predict_txt_path):
            ...
            for row in rows:
                ...
                #
                # xmin = int(xmin * x_factor)
                # ymin = int(ymin * y_factor)-10
                # xmax = int(xmax * x_factor)
                # ymax = int(ymax * y_factor)-10


                cv2.rectangle(frame, (int(xmin), int(ymin)), (int(xmax), int(ymax)-3), (255, 120, 203), 2)
                cv2.putText(frame, f"{i}", org=(int(xmin)+3, int(ymin)-3), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
                            color=(255, 120, 203), thickness=2, lineType=cv2.LINE_AA)
                ...
                print(f"{text}")
                cv2.putText(frame, text, org=(100, 150), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
